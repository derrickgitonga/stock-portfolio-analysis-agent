...
export default function OpenStocksCanvas() {
  ...
  useCopilotAction({
    ...
    renderAndWaitForResponse: ({ args, respond, status }) => {
      useEffect(() => {
        if (args?.investment_summary?.percent_allocation_per_stock &&
          args?.investment_summary?.percent_return_per_stock &&
          args?.investment_summary?.performanceData) {

          console.log("Auto-updating chart with data:", args.investment_summary);
          ...
          // Auto-respond to complete the action if not already completed
